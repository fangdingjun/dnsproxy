...
#ifdef DEBUG
    printf("recv %d bytes from %s:%d\n", d->msg_len, inet_ntoa(d->client_addr.sin_addr), ntohs(d->client_addr.sin_port));
#endif
    send_to_server(d);
    return 0;
}
...
int send_to_server(struct msg_data *d)
{
    ...
    for(i=0;;i++){
        ...
        if (sendto
                (d->srv_fd, d->msg_buf, d->msg_len, 0, (struct sockaddr *) &saddr,
                 sizeof(saddr)) < 0) {
...
#endif
#ifdef DEBUG
    printf("send to %s:%d\n", inet_ntoa(d->client_addr.sin_addr), htons(d->client_addr.sin_port));
#endif
    close(d->srv_fd);
    memset(d, 0, sizeof(struct msg_data));
...
#else
    printf("select version dnsproxy.\n");
#endif
