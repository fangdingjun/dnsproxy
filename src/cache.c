...
int is_exists(sqlite3 * db, char *tbl_name, char *name, int type,
              char *rdata);
...
int cache_store(sqlite3 * db, char *tbl_name, struct dns_rr *r)
{
    ...
    for (r1 = r; r1 != NULL; r1 = r1->next) {
        ...
        if (is_exists(db, tbl_name, r1->name, r1->type, (char *) r1->rdata)
            > 0) {
                           ...
                           int qtype)
{
    ...
    if (qname == NULL || db == NULL || tbl_name == NULL)
        return NULL;
              ...
              char *rdata)
{
    ...
    sprintf(sql,
            "select count(domain) as d from %s where domain = '%s' and  rrtype = %d and expired > %ld",
    ...
    if (num > 0 && num < 10) {
        if (type == RR_A || type == RR_AAAA) {
            ...
            sqlite3_finalize(stmt);
            sprintf(sql,
                    "select count(domain) from %s where domain = '%s' and "
